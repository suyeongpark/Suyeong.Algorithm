...
namespace Suyeong.Algorithm.Diff.Text
{
    public static class TextDiffer
    {
        ...
        public static DiffResultViewCollection DiffTexts(
            IEnumerable<string> mainTexts, 
            IEnumerable<string> subTexts, 
            out DiffResultCollection mainResults, 
            out DiffResultCollection subResults, 
            double similarThreshold = SIMILAR_THRESHOLD
        )
        {
            ...
            GetDiffResultDic(
                ...
                similarLimit: similarThreshold,
                mainResults: out mainResults,
                subResults: out subResults
            );

            return ConvertResultToViews(mains: mainResults, subs: subResults);
        }
        ...
        static void GetDiffResultDic(
            List<DiffSentence> mainSentences, 
            List<DiffSentence> subSentences, 
            double similarLimit, 
            out DiffResultCollection mainResults, 
            out DiffResultCollection subResults
        )
        {
            DiffResultDictionary resultDicMain = new DiffResultDictionary();
            DiffResultDictionary resultDicSub = new DiffResultDictionary();
            ...
            foreach (DiffSentence main in mainSentences)
            {
                ...
                for (int i = lastIndex + 1; i < subSentences.Count; i++)
                {
                    ...
                    {
                        resultDicMain.Add(main.Index, new DiffResult(
                            index: main.Index, 
                            diffType: DiffType.Same, 
                            main: main, 
                            sub: sub, 
                            sameTexts: main.Texts.ToList(), 
                            modifiedTexts: new List<string>())
                        );

                        resultDicSub.Add(sub.Index, new DiffResult(
                            index: sub.Index, 
                            diffType: DiffType.Same, 
                            main: sub, 
                            sub: main, 
                            sameTexts: sub.Texts.ToList(), 
                            modifiedTexts: new List<string>())
                        );
                    ...
                    else
                    {
                        ...
                        {
                            GetSameAndModifiedTexts(
                                mainTexts: main.Texts, 
                                subTexts: sub.Texts, 
                                sameTexts: out sameTexts, 
                                modifiedTexts: out modifiedTexts
                            );
                ...
                if (!find)
                {
                    resultDicMain.Add(main.Index, new DiffResult(
                        ...
                        main: main, 
                        sub: new DiffSentence(), 
                        sameTexts: new List<string>(), 
                        modifiedTexts: main.Texts.ToList())
                    );
                }
            }
            ...
            foreach (DiffSentence sentence in subSentences)
            {
                ...
                {
                    resultDicSub.Add(sentence.Index, new DiffResult(
                        index: sentence.Index, 
                        diffType: DiffType.Added, 
                        main: sentence, 
                        sub: new DiffSentence(), 
                        sameTexts: new List<string>(), 
                        modifiedTexts: sentence.Texts.ToList())
                    );
                }
            }

            // 순서가 꼬였으므로 순서대로 정렬한다.
            mainResults = new DiffResultCollection(resultDicMain.Values);
            subResults = new DiffResultCollection(resultDicSub.Values.OrderBy(result => result.Index));
        }

        static void GetSameAndModifiedTexts(
            string[] mainTexts, 
            string[] subTexts, 
            out List<string> sameTexts, 
            out List<string> modifiedTexts
        )
        {
            sameTexts = new List<string>();
            modifiedTexts = new List<string>();
        ...
        static DiffResultViewCollection ConvertResultToViews(
            List<DiffResult> mains, 
            List<DiffResult> subs
        )
        {
